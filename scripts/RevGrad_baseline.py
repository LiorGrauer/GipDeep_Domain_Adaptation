...
train_source_dset = datasets.WSI_REGdataset(DataSet='HAEMEK')
...
def train(epoch, model):
    ...
    while i <= len_source_loader:
        ...
        _, clabel_src = model(source_data, alpha = alpha)
        ...
        if i % opt.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tlabel_Loss: {:.6f}'.format(
                epoch, i * len(source_data), len_source_dataset,
                100. * i / len_source_loader, label_loss.item()))
        i = i + 1
        
        #tensorboard
        #train_total_domain_loss += domain_loss_total
        train_total_label_loss += loss_total
