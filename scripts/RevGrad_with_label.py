...
def train(epoch, model):
    ...
    while i <= len_source_loader:
        ...
        label_loss_src = loss_class(clabel_src, source_label.ravel())
        ...
        label_loss_tgt = loss_class(clabel_tgt, target_label.ravel()) #added
        domain_loss_tgt = loss_domain(dlabel_pred_tgt, dlabel_tgt)

        label_loss_total= label_loss_src + label_loss_tgt #added
        domain_loss_total = domain_loss_src + domain_loss_tgt
        loss_total = label_loss_total + domain_loss_total
        ...
        if i % opt.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tlabel_Loss: {:.6f}\tdomain_Loss: {:.6f}'.format(
                ...
                100. * i / len_source_loader, label_loss_total.item(), domain_loss_total.item()))
        i = i + 1
        
        #tensorboard
        train_total_domain_loss += domain_loss_total
        train_total_label_loss += label_loss_total
